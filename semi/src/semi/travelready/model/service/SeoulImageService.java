...
Connection conn=JDBCTemplate.getConnection();
	...
	public SeoulImageFile imageDown(int imageNo) {
		Connection conn=JDBCTemplate.getConnection();
		SeoulImageFile sif =new SeoulImageDao().imageDown(conn,imageNo);
		JDBCTemplate.close(conn);
		return sif;
		
	}

	public SeoulImagePageData searchList(int currentPage, String search) {
		Connection conn=JDBCTemplate.getConnection();
		
		int recordCountPerPage=10;
		int naviCountPerPage=5;
		
		ArrayList<SeoulImageFile> list=new SeoulImageDao().getSearchCurrentPage(conn,currentPage,recordCountPerPage,search);
		SeoulImagePageData sipd=new SeoulImageDao().getSearchPageNavi(conn,currentPage,recordCountPerPage,naviCountPerPage,search);
