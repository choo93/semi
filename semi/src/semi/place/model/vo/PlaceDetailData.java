package semi.place.model.vo;

public class PlaceDetailData {
	private int SEQ_Index_TitleNo;
	private String index_type;
	private String detail_addr; 
	private String detail_tell;  
	private String detail_url;
	private String detail_onTime;
	private String detail_offday;
	private String detail_onday; 
	private String detail_notice;  
	private String detail_payment;   
	private String detail_disabled;    
	private String detail_utility;

	public PlaceDetailData() {
		super();
		// TODO Auto-generated constructor stub
	}

	public PlaceDetailData(int sEQ_Index_TitleNo, String index_type, String detail_addr, String detail_tell,
			String detail_url, String detail_onTime, String detail_offday, String detail_onday, String detail_notice,
			String detail_payment, String detail_disabled, String detail_utility) {
		super();
		SEQ_Index_TitleNo = sEQ_Index_TitleNo;
		this.index_type = index_type;
		this.detail_addr = detail_addr;
		this.detail_tell = detail_tell;
		this.detail_url = detail_url;
		this.detail_onTime = detail_onTime;
		this.detail_offday = detail_offday;
		this.detail_onday = detail_onday;
		this.detail_notice = detail_notice;
		this.detail_payment = detail_payment;
		this.detail_disabled = detail_disabled;
		this.detail_utility = detail_utility;
	}

	public int getSEQ_Index_TitleNo() {
		return SEQ_Index_TitleNo;
	}

	public void setSEQ_Index_TitleNo(int sEQ_Index_TitleNo) {
		SEQ_Index_TitleNo = sEQ_Index_TitleNo;
	}

	public String getIndex_type() {
		return index_type;
	}

	public void setIndex_type(String index_type) {
		this.index_type = index_type;
	}

	public String getDetail_addr() {
		return detail_addr;
	}

	public void setDetail_addr(String detail_addr) {
		this.detail_addr = detail_addr;
	}

	public String getDetail_tell() {
		return detail_tell;
	}

	public void setDetail_tell(String detail_tell) {
		this.detail_tell = detail_tell;
	}

	public String getDetail_url() {
		return detail_url;
	}

	public void setDetail_url(String detail_url) {
		this.detail_url = detail_url;
	}

	public String getDetail_onTime() {
		return detail_onTime;
	}

	public void setDetail_onTime(String detail_onTime) {
		this.detail_onTime = detail_onTime;
	}

	public String getDetail_offday() {
		return detail_offday;
	}

	public void setDetail_offday(String detail_offday) {
		this.detail_offday = detail_offday;
	}

	public String getDetail_onday() {
		return detail_onday;
	}

	public void setDetail_onday(String detail_onday) {
		this.detail_onday = detail_onday;
	}

	public String getDetail_notice() {
		return detail_notice;
	}

	public void setDetail_notice(String detail_notice) {
		this.detail_notice = detail_notice;
	}

	public String getDetail_payment() {
		return detail_payment;
	}

	public void setDetail_payment(String detail_payment) {
		this.detail_payment = detail_payment;
	}

	public String getDetail_disabled() {
		return detail_disabled;
	}

	public void setDetail_disabled(String detail_disabled) {
		this.detail_disabled = detail_disabled;
	}

	public String getDetail_utility() {
		return detail_utility;
	}

	public void setDetail_utility(String detail_utility) {
		this.detail_utility = detail_utility;
	}
	
	
}
