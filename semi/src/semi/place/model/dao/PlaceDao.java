...
public class PlaceDao {
	...
	public ArrayList<RankListData> getListData(Connection conn, int recordCountPerPage, int currentPage, String search,
			String type) {
		...
		ArrayList<RankListData> list = new ArrayList<RankListData>();
		try {
			...
			while (rset.next()) {
				RankListData rld = new RankListData();
				rld.setIndex_BasicInfo(rset.getString("index_basicInfo"));
				rld.setIndex_Image(rset.getString("index_image"));
				rld.setIndex_Title(rset.getString("index_title"));
				rld.setIndex_Ondate(rset.getDate("index_Ondate"));
				rld.setIndex_TitleNo(rset.getInt("index_TitleNo"));
				rld.setIndex_Tags(rset.getString("index_Tags"));
				rld.setList_Element(rset.getString("list_Element"));
	...
	public String getPageNavi(Connection conn, int naviCountPerPage, int recordCountPerPage, int currentPage,
		...
		// 게시물의 토탈 개수를 구해야 함
				...
				if (needPrev) { // 시작이 1페이지가 아니라면!
					sb.append("<li> <a href='/placeRankList?currentPage=" + (startNavi - 1) +"&serarch="+search+"&type="+type+  "'> « </a> </li>");
				} else { // 시작 페이지가 1페이지라면 !
					sb.append("<li class='disabled'> <span>«</span> </li>");
				}
				for (int i = startNavi; i <= endNavi; i++) {
					if (i == currentPage) {
						sb.append("<li class='active'><a href='/place RankList?currentPage=" + i +"&serarch="+search+"&type="+type+ "'>" + i + "</a></li>");
					} else {
						sb.append("<li> <a href='/placeRankList?currentPage=" + i +"&serarch="+search+"&type="+type+  "'> " + i + " </a> </li>");
					}
				}
				if (needNext) { // 끝페이지가 아니라면!
					sb.append("<li><a href='/placeRankList?currentPage=" + (endNavi + 1) +"&serarch="+search+"&type="+type+  "'> » </a></li>");
				}else {
					sb.append("<li class='disabled'> <span>»</span> </li>");
				}
	...
	public PlaceDetailData placeSelect(Connection conn, int titleNo) {
		PreparedStatement pstmt = null;
		ResultSet rset = null;
