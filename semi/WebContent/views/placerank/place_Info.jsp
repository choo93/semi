...
<body id="scroll">
	<header
		style="position: fixed; width: 50px; border-right: 1px solid #ddd; height: 100%;">
		<div id="click">
			<a href="#" class="click"><img src="/image/main/menu.png"
				style="width: 30px; margin-top: 10px; margin-left: 10px; background-color: white; z-index: 999;"></a>
		</div>
		<div id="click-open" style="display: none;">
			<a href="http://localhost"><img src="/image/main/logo.png"
				id="main_logo"
				style="width: 260px; margin-left: 20px; margin-top: 20px;"></a>
...
a, ul>li {
	font-size: 1rem !important;
			...
			<% if(user==null){ %>
			<div id="login_join">
				<a href="/views/main/login.jsp" class="main-login"
					style="font-size: 1.5rem !important;">로그인</a> <a
					href="/views/main/joinus.jsp" style="font-size: 1.5rem !important;">회원가입</a>
			</div>
			<% } else { %>
			<div id="login_join" class="login_after">
				<% if(user.getUserId().equals("admin")){ %>
				<p style="margin-bottom: 8px; font-size: 1.5rem;">
					<span style="font-weight: bold;"><%=user.getUserName()%></span>로
					로그인 하셨습니다
				</p>
				<% } else { %>
				<p
					style="margin-bottom: 8px; margin-left: -20px; font-size: 1.5rem;">
					<span style="font-weight: bold;"><%=user.getUserName()%></span>님
					환영합니다
				</p>
				<a href="/myPage"
					style="margin-left: -19px; margin-right: 15px; font-size: 1.5rem !important;">마이페이지</a>
				<% } %>
		...
		<div class="container">
			<div class="container-fluid">
				...
				<div id="carousel-generic" class="carousel slide">
					...
					<!-- 컨트롤러 부분 -->
					<a href="#carousel-generic" class="left carousel-control"
						data-slide="prev"> <span
						class="glyphicon glyphicon-chevron-left"></span>
					</a> <a href="#carousel-generic" class="right carousel-control"
						...
						<div class="panel-body">
							...
							<table style="height: 100%; width: 100%;">
								...
								<tr>
									<td>주소</td>
									<td>
										<!-- 03131 서울 종로구 삼일대로 464(윤현궁) --> <%=pr.getPlaceAddr()%>
									</td>
								</tr>
								...
								<tr>
									...
									<td>
										...
										href="<%=pr.getPlaceSite()%>">웹사이트 보기</a>
									</td>
								</tr>
								<%}if(pr.getPlaceOntime()!=null){ %>
								<tr>
									<td>이용시간</td>
									<td><%=pr.getPlaceOntime()%></td>
								</tr>
								...
								<tr>
									<td>장애인 편의시설</td>
									<td><%=pr.getPlaceUtility()%></td>
								</tr>
								<%}if(pr.getPlaceNotice()!=null){ %>
								<tr>
									<td>이것만은 꼭!</td>
									<td><%=pr.getPlaceNotice()%></td>
								</tr>
								<%}if(pr.getPlacePayment()!=null){ %>
								<tr>
									<td>이용요금</td>
									<td><%=pr.getPlacePayment()%></td>
								</tr>
								<%} %>
					...
					<div id="collapseFour" class="panel-collapse collapse">
						<div class="panel-body" class="form-horizontal">
							...
							<div class="row form-group">
								...
								<input type="hidden" calss="form-control"
									value="<%=pr.getPlaceTitle() %>" id="title" /> <input
									type="hidden" class="form-control"
									value="<%=user.getUserId() %>" id="userId" /> <input
									type="hidden" class="form-control"
									value="<%=pr.getTitleNo() %>" id="titleNo" />
								...
								<div class="row">
									<table class="table">
										...
										<tr>
											...
											<td><%=prc.getUserComment() %><span
												class="glyphicon glyphicon-trash pull-right"
												onclick="return commentDelete(<%=prc.getTitleNo()%>,<%=prc.getReviewNo()%>);"></span></td>
											<%}else{ %>
							...
							<div class="row form-group">
								...
								<div class="col-xs-9 col-xs-offset-1">
									<input type="text" class="form-control"
										placeholder="로그인후 이용이 가능합니다." id="comment" name="comment"
										readonly onclick="login()" />
								</div>
								<script>
													function login() {
